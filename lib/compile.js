...
/**
 * @param {Loader} loader
 * @param {LoaderInput} input
 * @param {LoaderContext} context
 * @returns {Promise<Error | LoaderInput>}
 */
async function runLoader(loader, input, context) {
  try {
    return await (function LOADER_EXECUTION() {
      return loader.call(context, input);
    })();
  } catch (err) {
    return err;
  }
...
export default async function compile(file, compilerOptions) {
  ...
  async function compileModule(resourcePath, context) {
    ...
    const result = await resolveLoaders(matchedRule.use).reduceRight(
      (inputPromise, config) => {
        return inputPromise.then((input) => {
          ...
          return runLoader(config.loader, input, {
            ...context,
            getOptions() {
    ...
    if (result instanceof Error) {
      ...
      fs.writeFileSync(`${resourcePath}.h`, result);
    }
  }
