...
class Creator {
  ...
  async setup() {
    const pkgConfigPath = path.join(this.dir, 'package.json')
    const pkgConfig = require(pkgConfigPath)

    pkgConfig.name = this.name
    pkgConfig.version = '0.1.0'
    pkgConfig.private = true
    fs.writeFileSync(pkgConfigPath, JSON.stringify(pkgConfig, null, 2), { encoding: 'utf-8' })
