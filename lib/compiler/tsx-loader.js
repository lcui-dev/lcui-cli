...
function transformReactElement(el) {
  ...
  if (typeof el.type === "string") {
    ...
    node.name = el.type.name || el.type.displayName;
  } else {
    return;
  }
...
export default async function TsLoader(content) {
  ...
  const componentModule = await loader.importModule(this.resourcePath);
  const reactTree = transformReactElement(componentModule.default());
  return {
    name: "lcui-app",
    children: [
      ...assets.map((asset) => ({
