...
export default function fmt(...args: Value[]): ObjectBinding {
  ...
  const len = factory.createNumericVariable(`${str.__meta__.name}_len`, 8, CType.Size);
  ...
  ctx.body.push(`${str.__meta__.name} = malloc(sizeof(char) * ${len.__meta__.name})`);
  ctx.body.push(
    ...list.map((id, index) => `${index === 0 ? `strcpy` : `strcat`}(${str.__meta__.name}, ${id})`)
  );
  return str;
}
