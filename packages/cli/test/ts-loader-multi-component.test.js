...
describe("ts-loader — multi-component compilation", () => {
  ...
  it("compiles all exported and internal components with correct naming", async function () {
    ...
    assert.match(
      tsxH,
      /ui_create_widget_prototype\("app"/,
      "App should register widget prototype 'app'"
    );
    assert.match(
      helpersH,
      /void ui_register_app\(void\);/,
      "helpers.h should declare ui_register_app"
    );
    ...
    assert.match(
      tsxH,
      /ui_create_widget_prototype\("card"/,
      "Card should register widget prototype 'card'"
    );
    assert.match(
      helpersH,
      /void ui_register_card\(void\);/,
      "helpers.h should declare ui_register_card"
    );
    ...
    assert.match(
      tsxH,
      /helpers__my_button_proto/,
      "internal component should use __ separator: helpers__my_button"
    );
    assert.match(
      tsxH,
      /ui_create_widget_prototype\("helpers__my_button"/,
      "MyButton should register widget prototype 'helpers__my_button'"
    );
    assert.match(
      helpersH,
      /void ui_register_helpers__my_button\(void\);/,
      "helpers.h should declare ui_register_helpers__my_button"
    );
    assert.match(
      mainH,
      /ui_register_helpers__my_button\(\);/,
      "main.h should call ui_register_helpers__my_button()"
    );
    ...
    assert.match(
      tsxH,
      /typedef struct helpers__my_button_react_state/,
      "MyButton should have its own state type"
    );

    assert.match(tsxH, /ui_create_widget\("card"\)/, "App template should reference Card widget");
    assert.match(
      tsxH,
      /ui_create_widget\("helpers__my_button"\)/,
      "App template should reference MyButton widget with prefix"
    );
    ...
    assert.match(
      helpersC,
      /helpers__my_button_init/,
      ".c file should contain MyButton init function"
    );
  });
});
